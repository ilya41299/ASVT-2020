...
void print(std::vector<K> &result_implicants)
{
...
void get_result(std::vector<std::vector<K>> &string_coefficients)
{
	...
	while (not_yet(string_coefficients))
	{
		...
		for (size_t i = 0; i < string_coefficients.size(); i++)
		{
			for (size_t j = 0; j < string_coefficients[i].size(); j++)
			{
				if (string_coefficients[i][j].is_active && string_coefficients[i][j].deg < min_deg)
				{
					min_deg = string_coefficients[i][j].deg;
				}
			}
			if (min_deg != 10) break;
		}
		...
		for (size_t i = 0; i < string_coefficients.size(); i++)
		{
			for (size_t j = 0; j < string_coefficients[i].size(); j++) 
			{
				if (string_coefficients[i][j].is_active && string_coefficients[i][j].deg == min_deg)
				{
					implicant_and_rang.push_back(std::make_pair(string_coefficients[i][j], 1));
					is_check = true;
				}
		...
		for (size_t i = 0; i < string_coefficients.size(); i++)
		{
			for (size_t j = 0; j < string_coefficients[i].size(); j++) 
			{
				if (string_coefficients[i][j].is_active) 
				{
					...
					{
						if (implicant_and_rang[i].first == string_coefficients[i][j])
						{
							implicant_and_rang[i].second++;
						}
					}
				}
			}
...
int main()
{
	...
	get_result(string_coefficients);
}
